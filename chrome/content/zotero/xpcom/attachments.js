...
Zotero.Attachments = new function () {
	...
	this.STORAGE_PLACEHOLDER = 'storage:';
	 ...
	 * @param {String[]} [required] - Array of required options ["title", "linkMode", "contentType"]
	 */
	function ensureOptions(options, ...requiredOptions) {
		for (let o of requiredOptions) {
			if (!options[o]) throw new Error(`'${o}' not provided`);
		}

		if (options.parentItemID && options.collections) {
		...
		try {
			yield Zotero.DB.executeTransaction(function* () {
				...
				attachmentItem = _createAttachmentItem({
					libraryID,
					title: title !== undefined ? title : newName,
	...
	this.linkFromFileWithRelativePath = async function (options) {
		...
		ensureOptions(options, "path", "title", "contentType");
		...
		var attachmentItem = await _addToDB(Object.assign(options, {
			file: path,
			linkMode: this.LINK_MODE_LINKED_FILE,
		}));
		...
		// index it
		var file = this.resolveRelativePath(attachmentItem.libraryID, path);
		if (file && await OS.File.exists(file)) {
			try {
				await _postProcessFile(attachmentItem, file, contentType);
			}
			catch (e) {
				Zotero.logError(e);
			}
	...
	this.importSnapshotFromFile = Zotero.Promise.coroutine(function* (options) {
		...
		ensureOptions(options, "file", "parentItemID");
		...
		var attachmentItem, destDir, newPath;
		try {
			yield Zotero.DB.executeTransaction(function* () {
				// Create a new attachment
				attachmentItem = _createAttachmentItem(Object.assign(options, {
					file: 'storage:' + fileName,
					linkMode: this.LINK_MODE_IMPORTED_URL,
				}));
				// DEBUG: This now sets the accessDate because it uses _createAttachmentItem
	...
	this.createURLAttachmentFromTemporaryStorageDirectory = async function (options) {
		ensureOptions(
			options, "directory", "libraryID", "filename", "url", "contentType"
		);
		...
		var attachmentItem;
		try {
			attachmentItem = await _addToDB(Object.assign(options, {
				file: 'storage:' + options.filename,
				title: options.title !== undefined ? options.title : options.filename,
				linkMode: this.LINK_MODE_IMPORTED_URL,
	...
	this.linkFromDocument = Zotero.Promise.coroutine(function* (options) {
		...
		ensureOptions(options, "document");
	...
	this.importFromDocument = Zotero.Promise.coroutine(function* (options) {
		...
		try {
			...
			var attachmentItem = yield _addToDB(Object.assign(options, {
				file: 'storage:' + fileName,
				url,
				title,
				contentType,
			...
			var destDir = this.getStorageDirectory(attachmentItem).path;
	...
	this.getBaseDirectoryRelativePath = function (libraryID, path) {
		...
		var basePath = Zotero.Attachments.getBasePathByLibrary(libraryID);
		if (!basePath) {
			return path;
		}
	...
	this.resolveRelativePath = function (libraryID, path) {
		...
		var basePath = Zotero.Attachments.getBasePathByLibrary(libraryID, path);
		if (!basePath) {
			Zotero.debug(`No base attachment path set -- can't resolve '${path}' in library '${libraryID}'`, 2);
			return false;
		}

		return this.fixPathSlashes(OS.Path.join(
			basePath, path.substr(Zotero.Attachments.BASE_PATH_PLACEHOLDER.length)
		));
	};
	 ...
	 * @return {Boolean}
	 */
	this.setBasePathByLibrary = function(libraryID, newBase) {
		Zotero.debug(`Setting base attachment path for library '${libraryID}': '${newBase}'`);
	...
	/**
	 * Get whether attachments within ``libraryID``'s base attachment directory
	 ...
	 * null for ``saveRelative``.
	 *
	 * @param {Integer} libraryID
	 * @param {String|null} saveRelative - New base attachment path for ``libraryID`` or clear it if null
	 * @return {Boolean}
	 */
	...
	/**
	 * Create a new item of type 'attachment'
	 *
	 * @param {Object} options
	 * @param {nsIFile|String} [options.file]
	 * @param {String} [options.url]
	 * @param {Integer} [options.libraryID]
	 * @param {String} options.title
	 * @param {Number} options.linkMode
	 * @param {String} options.contentType
	 * @param {String} [options.charset]
	 * @param {Number} [options.parentItemID]
	 * @param {String[]|Number[]} [options.collections]
	 * @param {Object} [options.saveOptions]
	 * @return {Zotero.Item} - A populated but unsaved attachment item
	 */
	function _createAttachmentItem(options) {
		...
		if (libraryID) {
			...
			let { libraryID: parentLibraryID, key: _ }
	...
	function _addToDB(options) {
		return Zotero.DB.executeTransaction(function* () {
			// Create a new attachment
			var attachmentItem = _createAttachmentItem(options);

			yield attachmentItem.save(options.saveOptions);
